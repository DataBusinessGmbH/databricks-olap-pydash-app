...
class DatabricksSparkBackend:
    ...
    def execute(self, request: OlapQueryRequest) -> pd.DataFrame:
        ...
        Behaviour
        ---------
        * Query backend fresh for each execution (no caching of aggregated results).
    ...
    def _build_flat_table(self) -> pd.DataFrame:
        ...
        for dim in self._model.dimensions:
            dim_name = self._qualified_table_name(dim)
...
class DatabricksSqlBackend:
    ...
    def flat_table(self) -> pd.DataFrame:
        if self._flat is None:
            LOGGER.info("Building SQL flat table for model=%s", self._model.name)
            self._flat = self._build_flat_table()
            LOGGER.info("SQL flat table built: shape=%s", self._flat.shape)
        return self._flat
    ...
    def execute(self, request: OlapQueryRequest) -> pd.DataFrame:
        ...
        # Build SQL query with only requested dimensions (bypasses flat_table cache)
        sql = self._build_flat_sql(request)
        ...
        agg_map = {m.name: m.default_agg for m in self._model.metrics if m.name in metrics}
        agg_kwargs = {m: (m, func) for m, func in agg_map.items()}
    ...
    def _build_flat_table(self) -> pd.DataFrame:
        sql = self._build_flat_sql(request=None)  # request=None → include all dimensions
    ...
    def _build_flat_sql(self, request: OlapQueryRequest | None = None) -> str:
        """
        ...
        for dim in self._model.dimensions:
            ...
            if dim.dim_key in requested_cols or include_all_dims==True:
                ...
                dim_key_include = True      

            # Only join dimension if it has attributes to include
            if not dim_attrs_to_include and dim_key_include is False:
                continue
